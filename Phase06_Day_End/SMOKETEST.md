...
2) Dry-run checks (no system impact)
   - Run each script and abort at confirmation prompt:
     - `./clear_terminal_history.sh` (type anything except CLEAR)
     - `sudo ./clear_history.sh` (type anything except CLEAR)
     - `sudo ./system_cleanup.sh` (type anything except CLEAN)

3) Confirm warnings are shown
   - Root warning appears if `clear_terminal_history.sh` is run with sudo
