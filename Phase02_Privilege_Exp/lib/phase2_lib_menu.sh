#!/usr/bin/env bash
# lib/phase2_lib_menu.sh
set -euo pipefail

# ============================================================
# Purpose : Phase 2 menu + prompt helpers
# Version : 0.1.0
#
# Design goals:
...
# - PHASE_NAME / PHASE_VERSION used in headers when title omitted.
# ============================================================
...
_phase2_menu__is_tty() { [[ -t 1 ]]; }

_phase2_menu__color_enabled() {
  case "${PHASE2_MENU_COLOR}" in
    1|true|yes) return 0 ;;
    0|false|no) return 1 ;;
    auto) _phase2_menu__is_tty ;;
    *) _phase2_menu__is_tty ;;
  esac
}
...
# ---------- Formatting helpers ----------
phase2_menu__header() {
  # Usage: phase2_menu__header ["Title"] ["Subtitle"]
...
# ---------- Input helpers ----------
phase2_menu__pause() {
  # Usage: phase2_menu__pause ["prompt"]
...
phase2_menu__ask() {
  ...
  # Usage: val="$(phase2_menu__ask "Enter team number" "21")"
...
phase2_menu__choose() {
  ...
  #
  # Usage:
  #   idx="$(phase2_menu__choose "Select action" 1 "Run" "Show" "Exit")"
  #
  # Behavior:
  ...
  if (( ${#options[@]} == 0 )); then
    _phase2_menu__warn "phase2_menu__choose called with no options"
    echo "0"
    return 0
  fi
...
phase2_menu__choose_kv() {
  # Menu from label/value pairs; returns VALUE (or empty on cancel).
  #
  # Usage:
  #   val="$(phase2_menu__choose_kv "Pick output" 1 "Services TXT" "./services.txt" "CSV" "./services.csv")"
  local title="${1:-Select}"; shift || true
  local default="${1:-0}"; shift || true

  if (( $# == 0 )); then
    _phase2_menu__warn "phase2_menu__choose_kv called with no pairs"
    echo ""
    return 0
  fi

  if (( $# % 2 != 0 )); then
    _phase2_menu__warn "choose_kv expects label/value pairs (even number of args)"
    echo ""
    return 1
  fi
  ...
  local idx
  idx="$(phase2_menu__choose "$title" "$default" "${labels[@]}")"
  if [[ "$idx" == "0" ]]; then
    echo ""
    return 0
  fi
...
phase2_menu__choose_multi() {
  ...
  #
  # Usage:
  #   picks="$(phase2_menu__choose_multi "Select checks" "Ping" "DNS" "HTTP")"
  local title="${1:-Select items}"; shift || true
  local options=("$@")

  if (( ${#options[@]} == 0 )); then
    _phase2_menu__warn "phase2_menu__choose_multi called with no options"
    echo ""
    return 0
  fi
